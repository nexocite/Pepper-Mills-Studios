...
// ------- Sticky Header (appears after slight scroll) -------
...
// ------- Background Carousel (cinematic lifestyle photos as BACKGROUND) -------
...
// ------- Hero Section (now uses BackgroundCarousel) -------
function Hero() {
  const heroImages = [
  ...
  return (
    <section id="experience" className="relative w-full py-24">
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(rgba(0,0,0,0.60),rgba(0,0,0,0.85)), url('https://images.unsplash.com/photo-1533449422022-6928940b0e91?q=80&w=2574&auto=format&fit=crop')",
        }}
      />
...
const platformCards = [
  {
    title: "Reversal Labs",
    desc: "HBOT, cryo, and red light therapies to accelerate recovery and cellular rejuvenation.",
    img: "https://images.unsplash.com/photo-1550439062-609e1531270e?q=80&w=2484&auto=format&fit=crop",
  },
  {
    title: "Mind Sanctuary",
    desc: "Minimalist meditation and breathwork studios crafted for cognitive clarity.",
    img: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?q=80&w=2484&auto=format&fit=crop",
  },
  {
    title: "Metabolic Bar",
    desc: "Nutrient-dense cuisine engineered for energy, focus, and longevity.",
    img: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?q=80&w=2484&auto=format&fit=crop",
  },
  {
    title: "NuVentures Incubator",
    desc: "Deep work bays and venture tooling to translate clarity into creation.",
    img: "https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2484&auto=format&fit=crop",
  },
];
...
function AIArchitect() {
  ...
  const systemPrompt = useMemo(
    () =>
      `You are an expert performance and longevity coach for Pepper Mills Studios. Your tone is knowledgeable, inspiring, and precise. A user's goal is: "${goal}". Create a sample 3-day wellness protocol tailored to this goal using only the modalities available at Pepper Mills: Reversal Labs (HBOT, cryo, red light), Mind Sanctuary (Meditation, breathwork), Metabolic Bar (meals), and NuVentures Incubator (deep work). Structure the output with Markdown for Day 1, Day 2, Day 3.`,
    [goal]
  );
...
- **NuVentures:** 2× 60-min focus blocks + founder walk debrief`;
  ...
  async function generateProtocol() {
    if (!goal.trim()) {
      setOutput("Please enter a goal first.");
      return;
    }
    setLoading(true);
    setOutput(null);
    try {
      ...
      setOutput(mockResponse(goal));
      // In production, call your serverless endpoint that proxies Gemini.
    } catch (e) {
      setOutput("We couldn't reach the generator. Showing a local sample instead.\n\n" + mockResponse(goal));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/40 p-6 backdrop-blur">
      <motion.div variants={staggerParent} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }}>
        <motion.h4 variants={fadeUp} className="text-2xl font-semibold text-white">AI Protocol Architect</motion.h4>
        <motion.p variants={fadeUp} className="mt-1 text-sm text-white/70">Describe your goal and generate a 3-day protocol using our modalities.</motion.p>
        ...
        <motion.div variants={fadeUp} className="mt-4">
          {output && (
            <pre className="whitespace-pre-wrap rounded-2xl border border-white/10 bg-black/50 p-4 font-mono text-[13px] leading-relaxed text-white/90">{output}</pre>
          )}
        </motion.div>
      </motion.div>
    </div>
...
const tiers = [
  {
    ...
    popular: false,
    features: [
      "Mind Sanctuary access",
      "Metabolic Bar member pricing",
      "Monthly protocol refresh",
      "Community events",
    ],
  },
  {
    ...
    features: [
      ...
      "2× HBOT or Cryo sessions / week",
      "Red Light bundle",
      "Weekly deep-work bay reservation",
    ],
  },
  {
    name: "Luminary",
    ...
    features: [
      ...
      "Unlimited Cryo + Red Light",
      "4× HBOT sessions / month",
      "Concierge scheduling + founder labs",
    ],
  },
];
        ...
        <motion.div variants={staggerParent} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {tiers.map((t) => (
            <motion.div key={t.name} variants={fadeUp} className={`relative rounded-3xl border bg-gradient-to-b from-white/5 to-white/[0.03] p-6 ${t.popular ? "border-cyan-400/40 shadow-[0_0_60px_rgba(34,211,238,0.15)]" : "border-white/10"}`}>
              {t.popular && (
                <div className="absolute -top-3 right-4 rounded-full border border-cyan-400/40 bg-black/60 px-3 py-1 text-xs text-cyan-300">Most Popular</div>
              )}
...
const testimonials = [
  {
    quote: "We replaced hustle with design. My recovery scores and investor updates both improved in the same quarter.",
    name: "Elena Ruiz",
    title: "Founder, Biotech Startup",
    img: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=800&auto=format&fit=crop",
  },
  {
    quote: "Deep-work bays + HBOT changed my cadence. I ship faster without the rollercoaster.",
    name: "Marcus Lee",
    title: "CTO, Fintech",
